...
const SkillsSection = () => {
  ...
  return (
    <section 
      id="skills" 
      className={`py-20 relative ${theme === 'dark' ? 'bg-transparent' : 'bg-transparent'}`}
    >
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        >
          ...
          <div 
            className={`h-1 w-20 mx-auto mb-6 ${theme === 'dark' ? 'bg-gradient-to-r from-blue-500 to-purple-500' : 'bg-gradient-to-r from-blue-600 to-purple-600'}`}
          ></div>
          <p className={`${theme === 'dark' ? 'text-gray-300' : 'text-gray-600'}`}>
            Explore my technical expertise, professional abilities, and specialized knowledge
          </p>
        </motion.div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                  {category.skills.map((skill, index) => (
                    <motion.div
                      ...
                      whileHover={{ y: -5 }}
                      className={`p-4 rounded-lg flex items-center justify-center text-center ${
                        theme === 'dark' 
                          ? 'bg-gradient-to-br from-purple-900/40 to-blue-900/40 border border-purple-800/30' 
                          : 'bg-gradient-to-br from-purple-100 to-blue-100 border border-purple-200/50'
                      } transition-all`}
                    >
                      <span className="font-medium">{skill}</span>
                    </motion.div>
